...
public class LongestCommonSubSequence {
     ...
     * @param j 序列Y的长度
     */
    private void recursionSolutionLcs(char[] x, char[] y, int[][] c, int i, int j) {
        if (i == 0 || j == 0) {
            c[i][j] = 0;
            return;
        }
    ...
    @Test
    public void testLcs() {
        Data data = lcsLength(x.toCharArray(), y.toCharArray());
        // print table c
    ...
    public void testRecursionSolutionLcs() {
        ...
        recursionSolutionLcs(x.toCharArray(), y.toCharArray(), c, x.length(), y.length());
